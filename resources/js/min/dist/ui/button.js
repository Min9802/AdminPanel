...
const buttonVariants = cva("group inline-flex items-center justify-center text-sm font-medium transition-colors focus-visible:border-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50", {
    variants: {
        variant: {
...
const Button = React.forwardRef(({ className, variant, size, color, asChild = false, tooltip = false, children, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    return (_jsxs(Comp, { className: classNames(cn(buttonVariants({ variant, size, color, className })), tooltip && "relative"), ref: ref, ...props, children: [children, tooltip ? (_jsxs("div", { className: "absolute bottom-3 flex flex-col items-center hidden mb-6 group-hover:flex animate-slide-up-fade", children: [_jsx("span", { className: "relative rounded-md z-10 p-1 text-xs leading-none text-white whitespace-no-wrap bg-black shadow-lg dark:bg-white dark:text-dark", children: tooltip }), _jsx("div", { className: "w-3 h-3 -mt-2 rotate-45 bg-black dark:bg-white" })] })) : null] }));
});
